...
-- Main zones:
--   Phase 1, Phase 2, SBS Lot, Sports Complex Lot
-- Phase 2 sub-zones:
--   Library Lot, Graduation Square, MSB Lot, Engineering Lot, Gate E Lot
-- ============================================
...
-- ============================================
-- SAMPLE DATA - PARKING SPACES (Phase 1 only for brevity)
-- ============================================
...
FROM parking_zones pz,
    GENERATE_SERIES(1, 50) gs
WHERE pz.zone_code = 'PH1' ON CONFLICT (zone_id, space_number) DO NOTHING;
-- ============================================
...
SELECT d.id,
    ...
    (
        SELECT id
        FROM users
        WHERE email = 'security1@university.edu'
    ),
    true
FROM drivers d
...
SELECT d.id,
    ...
    (
        SELECT id
        FROM users
        WHERE email = 'security2@university.edu'
    ),
    true
FROM drivers d
    JOIN users u ON d.user_id = u.id
...
SELECT u.first_name,
    ...
    COUNT(
        CASE
            WHEN i.status = 'resolved' THEN 1
        END
    ) as resolved,
    COUNT(
        CASE
            WHEN i.status IN ('reported', 'under_review') THEN 1
        END
    ) as pending,
...
SELECT pz.zone_name,
    ...
    (
        SELECT COUNT(*)
        FROM infringements
        WHERE parking_zone_id = pz.id
            AND status IN ('reported', 'under_review')
    ) as pending_infringements
FROM parking_zones pz
