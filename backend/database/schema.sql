...
CREATE TYPE infringement_status AS ENUM (
    'reported',
    'under_review',
    'resolved',
    'dismissed',
...
CREATE TABLE vehicles (
    ...
    -- Security person who registered it
...
CREATE TABLE parking_spaces (
    ...
    reserved_for_user_id UUID REFERENCES drivers(id),
    -- If reserved for specific driver
...
CREATE TABLE alerts (
    ...
    -- 'zone_full', 'high_occupancy', 'maintenance', etc.
...
-- See backend/scripts/migrate_alerts_table.py for the migration that
-- upgrades an existing 'description'-column DB to this richer shape.
-- ============================================
-- INFRINGEMENTS TABLE
-- ============================================
CREATE TABLE infringements (
    ...
    parking_zone_id UUID REFERENCES parking_zones(id),
    parking_space_id UUID REFERENCES parking_spaces(id),
    infringement_type VARCHAR(100) NOT NULL,
    -- 'overstay', 'no_permit', 'unauthorized_zone', etc.
    description TEXT,
...
SELECT pz.id,
    ...
    COUNT(
        CASE
            WHEN ps.status = 'reserved' THEN 1
        END
    ) as reserved_spaces,
    COUNT(
        CASE
            WHEN ps.status = 'cordoned' THEN 1
        END
    ) as cordoned_spaces,
    COUNT(
        CASE
            WHEN ps.status = 'maintenance' THEN 1
        END
    ) as maintenance_spaces,
    ROUND(
        100.0 * COUNT(
            CASE
                WHEN ps.status = 'occupied' THEN 1
...
SELECT d.id as driver_id,
    ...
    COUNT(
        CASE
            WHEN v.is_primary = true THEN 1
        END
    ) as primary_vehicles,
    COUNT(
        CASE
            WHEN v.is_active = true THEN 1
        END
    ) as active_vehicles
FROM drivers d
